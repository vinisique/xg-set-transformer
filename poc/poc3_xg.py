...
d = np.load("shots.npz", allow_pickle=True)
...
# ---------- tokens: chutador + jogadores do freeze-frame ----------
shooter = np.stack([sx / 120, sy / 80, dist / 50, angle,
                    np.ones(N), np.zeros(N), np.ones(N), header], 1)[:, None, :]
pdist = np.hypot(120 - px, py - 40)
others = np.stack([px / 120, py / 80, pdist / 50, np.zeros((N, P)),
                   mate, gk, np.zeros((N, P)), np.zeros((N, P))], -1)
tok = np.concatenate([shooter, others], 1).astype(np.float32)   # [N, 22, 8]
pad = np.concatenate([np.zeros((N, 1)), 1 - mask], 1) > 0.5     # True = ignorar

class DeepSets(nn.Module):
    def __init__(self, dim=32, drop=0.1):
        super().__init__()
    ...
    def forward(self, x, pad):
        ...
        return self.head((h * w).sum(1) / w.sum(1)).squeeze(-1)
